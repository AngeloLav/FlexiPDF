...
class PDFViewerActivity : AppCompatActivity(), PdfLoadCallback {
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        ...
        outState.putBoolean("currentLayoutIsFoldable", currentLayoutIsFoldable) // Salva anche lo stato del layout
    ...
    private fun updateLayout(shouldBeFoldable: Boolean) {
        // Se lo stato desiderato è già quello corrente, non fare nulla per evitare aggiornamenti inutili
        if (shouldBeFoldable == currentLayoutIsFoldable) {
            Log.d("PDFViewerActivity", "Layout già nello stato desiderato ($shouldBeFoldable). Ignoro l'aggiornamento.")
            return
        }
        ...
        if (shouldBeFoldable) {
            ...
            // Poiché `activity_pdf_viewer_foldable.xml` è il layout radice, manipoliamo i suoi figli.
            ...
            binding?.pdfFragmentRightContainer?.visibility = View.GONE // Nascondi il pannello destro
